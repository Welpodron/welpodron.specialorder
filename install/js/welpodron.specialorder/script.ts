((window) => {
  if (window.welpodron && window.welpodron.templater) {
    ...
    type SpecialOrderConfigType = {};

    type SpecialOrderPropsType = {
      sessid: string;
      element: HTMLElement;
      config?: SpecialOrderConfigType;
    };
    ...
    class SpecialOrder {
      ...
      constructor({ sessid, element, config = {} }: SpecialOrderPropsType) {
        if ((SpecialOrder as any).instance) {
          if (sessid) {
            (SpecialOrder as any).instance.sessid = sessid;
          }
          return (SpecialOrder as any).instance;
        }
        ...
        (SpecialOrder as any).instance = this;
      }
      ...
      add = async ({
        ...
        try {
          const response = await fetch(
            "/bitrix/services/main/ajax.php?action=welpodron%3Aspecialorder.Receiver.add",
            {
              method: "POST",
              body: data,
            }
